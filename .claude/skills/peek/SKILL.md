...
allowed-tools: Agent, Read, Write, Edit, Bash(node *), Bash(npm run dev), Bash(curl *), Bash(rm *), Bash(python *)
---
...
스크린샷은 한 장에 1,000~2,000토큰이고, 한 번 대화에 들어오면 그 대화가
끝날 때까지 **매 턴 다시 실린다.** 재 봤다 — peek 세 번에 이미지 136장,
19만 토큰이 한 대화에 쌓여 있었다. 사용량의 3분의 1이 여기서 났다.

그래서 화면을 여닫는 일은 `Agent`로 넘기고 **결론만 받는다.**

```
Agent(subagent_type: "general-purpose", model: "sonnet", prompt: "...")
```

심부름꾼에게는 무엇을 재고 무엇을 답할지 적어 준다. 돌아오는 것은
"03 화면에서 자판이 올라오면 버튼이 12px 잘린다" 같은 문장이지 이미지가
아니다. 이미지는 스크래치패드에 남으니 정말 볼 일이 생기면 그때 한 장 연다.

**예외는 눈으로만 판정되는 것뿐이다.** 여백의 균형, 글자의 인상, 색의
어울림 — 이건 한두 장을 직접 연다. 수십 장을 훑는 일은 예외가 아니다.
그건 심부름꾼이 할 일이다.

아래 내용은 그 심부름꾼에게 그대로 넘겨 주면 된다.
