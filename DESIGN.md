...
This contract governs one surface: the **MEGAFONT participant phone app** — the portrait
web app a visitor opens from the QR code beside the installation, rendered in a column
capped at 480px and installable to the home screen. Screens 00 through 08 are in scope.

Two sibling surfaces are out of scope. `/wall` is the 2.4 x 1.5 m landscape display
mounted on the campus building; it is the artwork rather than product chrome, is never
touched, and is read from 3 to 15 m, so phone-scale type and spacing do not transfer to
it. `/admin` is a read-only operator view.
<!-- design-md:claim-end -->

<!-- design-md:claim primary-tasks kind=user-outcomes count=5 lang=en -->
### Primary tasks

- Write one line for a public wall, up to 60 characters, on a personal phone.
- Choose how that line speaks — typeface, weight, width, slant, and colour — so the form of the sentence is authored rather than defaulted.
- Dock the phone into the object's slot, top edge first, while standing to face the wall.
- Read the result on the wall, where the line takes the whole display for up to 30 seconds.
- Take the phone back, which ends the loud reading and leaves the line to drift for three days before it disappears.
<!-- design-md:claim-end -->
