...
02 성격          당당한 / 차분한 / 다정한 / 발랄한. 화면을 채운 2×2 4분면
03 조율          빨간 면이 제목까지 덮고 그 위에 큰 견본(두둥실 뜬다).
                 아래 축 셋 — 크기 · 빠르기 · 무게. 축마다 눈금 다섯 칸 버튼.
                 빠른 쪽 두 칸은 기울기도 같이 준다(24도 · 12도)
04 한 줄         60자. 정렬은 가운데 고정, 기울기는 03의 '빠르기'가 정한다
                 자판을 내리면 화면이 물러서며 가장자리에서 파동이 번진다 —
                 적고 있던 것이 실은 소리였음이 그때 보인다.
                 화면을 누르면 다시 들어온다. 60자를 채우면 자판이 내려간다
05 색            왼쪽 글자색 · 오른쪽 줄 배경색. 동일 색 및 검정 배경 제외
06 미리보기      16:10 화면. CTA 안에서 전송·로딩·오류 재시도
07 도킹          위쪽부터 세로로 넣는 안내 + 작은 테스트 버튼
08 발화 중       브랜드 빨강 화면에 흰색 남은 시간. 최대 30초
...
별도 축이 하는 일이었다. 새 글꼴은 **정적**이라 이 칸에서만 무게 축이 진짜
굵기가 아니라 브라우저의 합성 굵기로 답한다. (실제 렌더 확인:
`node scripts/shoot-faces.mjs`)
