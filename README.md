...
캠퍼스 외벽에 올린다. 올라간 글은 사흘 동안 다른 말들 사이를 흘러다니다
...
03 메시지        입력창이 곧 외벽 미리보기. 60자. 색 10조합
04 미리보기      16:10 액자 — 실제 외벽과 같은 비율·같은 크기 계산
05 전송 중       진행바 (최소 800ms 노출)
06 도킹 안내     "홈에 꽂아주세요" + 꽂으면 무엇이 일어나는지
07 외벽에 떠 있음 10초 카운트다운. 고개를 들게 하는 화면
...
| `/wall` | 외벽 출력. 3트랙 풍경 + 트리거 시 10초 강조 |
| `/admin` | 저장된 메시지 목록 |
...
| 파일 | 담긴 것 |
|:--|:--|
| [src/lib/wall.ts](src/lib/wall.ts) | 외벽 크기(2.4×1.5m), 체류 기간(3일), 강조 시간(10초) |
...
체류 기간이나 강조 시간을 바꾸면 화면 카피와 외벽 필터가 함께 따라간다.
...
`?mock=1`로 돌기 때문에 캡처 과정의 전송이 실제 외벽에 닿지 않는다.
