 ...
 * 손. **멈춰 섰을 때만 든다.**
 *
 * 걷는 동안에는 몸통뿐이다 — 56px에서 팔까지 흔들면 무엇이 다리고 무엇이
 * 팔인지 안 갈린다. 메가폰트 앞에 서서 이야기하는 동안에만 손이 나온다.
 *
 * 옆모습은 보는 쪽 끝에 하나(몸 왼쪽 끝 17.13), 정면은 양쪽에 둘이다.
 * 몸과 같은 색이라 실루엣에서 툭 튀어나온 혹으로 읽힌다.
 */
export const HAND = { cy: 262.02, r: 39 };
export const HAND_SIDE = 17.13;
export const HAND_FRONT = [15.9, 349.33];
