...
export default function VoiceBubble({ text, bg, color, fontFamily, weight, width = 1, slant = 0, fontSize, align = 'center', size = 44, children }: Props) {
    ...
    return <div className="voice-bubble line-bubble" style={{ '--line-bg': bg, color, fontFamily, fontWeight: weight, fontVariationSettings: '"wght" ' + weight, textAlign: align, fontSize: fontSize ?? `max(14px, calc(${fitFontSize(text, { min: 3, max: 240 })} * 0.52 * ${scale} / ${Math.max(1, width)}))` } as CSSProperties}>
